...
export default function ProductDetailsScreen() {
  ...
  >(null);
  const [variantModalVisible, setVariantModalVisible] =
    useState<boolean>(false);

  // only used for gating the heart-icon
  ...
  const toggleFavorite = useCallback(async () => {
    ...
    try {
      ...
      // if the user is not logged in, your service can throw—
      // we catch here and pop the login modal:
  ...
  const addToCart = useCallback(async () => {
    ...
    Toast.show({
      ...
      text1: success ? 'Added to Cart' : 'Failed to Add',
    });

    if (success) {
      // jump to cart—remove this line if you'd rather stay here
      router.push('/cart');
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.price}>
            ₱{((defaultVar.price ?? product.price) || 0).toFixed(2)}
          </Text>
          ...
          {product.variants.length > 1 && (
            ...
            >
              <Text style={styles.variantText}>
                {defaultVar.size ?? 'Choose Variant'}
              </Text>
              <FontAwesome
                name="angle-down"
                size={18}
      ...
      <View
        style={[
          styles.footer,
          inStock ? styles.footerActive : styles.footerDisabled,
        ]}
      >
        <TouchableOpacity
        ...
        <TouchableOpacity
          style={styles.favoriteFooterBtn}
      ...
      {/* Only used if the user tries to heart without being logged in */}
      <LoginRequiredModal
